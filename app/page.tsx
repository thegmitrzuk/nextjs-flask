...
export default function Home() {
  const [isAgendaSubmitted, setIsAgendaSubmitted] = useState(false);

  const handleAgendaSubmit = () => {
    setIsAgendaSubmitted(true);
  };
  ...
  return (
    <main className="min-h-screen w-full bg-gradient-to-b from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800 flex flex-col items-center justify-center p-6 sm:p-8">
      <div className="max-w-3xl w-full flex flex-col items-center">
        ...
        {isAgendaSubmitted && <AudioRecorder isDisabled={false} />}
      </div>

      {!isAgendaSubmitted && <AgendaSaver onAgendaSubmit={handleAgendaSubmit} />}
